...
class VideoCommentDeleteDialog {
  ...
  late String _selectCode = '';
  late bool _isAbleDelete = false;
  ...
  VideoCommentDeleteDialog(this._tag, this._pageKey, this._dialogSKey) {
    for (int i = 0; i < InternationalLocalizations.commentDeleteTypeCodeList.length; i++) {
      _listCommentDelete.add(VideoCommentDeleteRadioBean(
          InternationalLocalizations.commentDeleteTypeCodeList[i], InternationalLocalizations.commentDeleteTypeNameList[i]));
    }
  }

  /// 初始化数据
  void initData(String id, String vid, String uid, OnSuccessListener onSuccessListener, {HandleDeleteCallBack? handleDeleteCallBack}) {
    _id = id;
    _vid = vid;
    _uid = uid;
    _isAbleDelete = false;
        ...
        children: <Widget>[
          ...
          Container(
            ...
            child: Text(
              ...
              style: TextStyle(fontSize: 11, color: Common.getColorFromHexString("A0A0A0", 1.0)),
            ),
          ),
          Expanded(
              child: Container(
  ...
  void _httpVideoCommentDel(BuildContext context) {
    if (this._handleDeleteCallBack != null) {
      this._handleDeleteCallBack?.call(true, false);
    }
    RequestManager.instance.videoCommentDel(_tag, _id ?? '', _vid ?? '', _uid ?? '', _selectCode ?? '').then((response) {
      if (response == null) {
        if (this._handleDeleteCallBack != null) {
          this._handleDeleteCallBack?.call(false, false);
        }
        return;
      }
      ...
      if (bean.status == SimpleResponse.statusStrSuccess) {
        ...
        if (_onSuccessListener != null) {
          _onSuccessListener?.call();
          if (this._handleDeleteCallBack != null) {
            this._handleDeleteCallBack?.call(false, true);
          }
        }
        Navigator.of(context).pop();
      } else {
