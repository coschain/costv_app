...
class RevenueCalculationUtil {
  ...
  ///获得结算奖金Vest
  static double getVideoRevenueVest(String votePower,
      dynamic_properties chainStateDgpoBean) {
    if (!TextUtil.isEmpty(votePower) &&
        chainStateDgpoBean != null &&
        !TextUtil.isEmpty(chainStateDgpoBean.weightedVpsPost) &&
        chainStateDgpoBean.poolPostRewards != null &&
        chainStateDgpoBean.poolPostRewards.value != null) {
      double vp = double.parse(votePower);
      double r1 =
      NumUtil.add(vp, double.parse(chainStateDgpoBean.weightedVpsPost));
      return NumUtil.divide(
          NumUtil.divide(
              NumUtil.multiply(vp, chainStateDgpoBean.poolPostRewards.value.toInt()),
              r1),
          cosUnit);
    } else {
      return 0;
  ...
  /// 获得总共收益Vest
  static double getTotalRevenueVest(String votePower, String vestGift,
      dynamic_properties chainStateDgpoBean) {
    if (!TextUtil.isEmpty(votePower) && !TextUtil.isEmpty(vestGift)) {
      return NumUtil.add(getVideoRevenueVest(votePower, chainStateDgpoBean),
          getGiftRevenueVest(vestGift));
    } else {
      return 0;
    }
  }
  ...
  static double getReplyVestByPower(String power, dynamic_properties chainStateDgpoBean) {
    if (!TextUtil.isEmpty(power) &&
        chainStateDgpoBean != null &&
        !TextUtil.isEmpty(chainStateDgpoBean.weightedVpsReply) &&
        chainStateDgpoBean.poolReplyRewards != null &&
        chainStateDgpoBean.poolReplyRewards.value != null) {
      double vp = double.parse(power);
      double r1 =
      NumUtil.add(vp, double.parse(chainStateDgpoBean.weightedVpsReply));
      return NumUtil.divide(
          NumUtil.divide(
              NumUtil.multiply(vp, chainStateDgpoBean.poolReplyRewards.value.toInt()),
              r1),
          cosUnit);
    } else {
      return 0;
  ...
  static double vestToRevenue(double revenue, ExchangeRateInfoData bean) {
    if (bean != null &&
        bean.cosusdt != null &&
        !TextUtil.isEmpty(bean.cosusdt.price)) {
      ...
      if (lan == 'cn') {
        if (bean.usdcny != null && !TextUtil.isEmpty(bean.usdcny.price)) {
          return NumUtil.multiply(
              NumUtil.multiply(revenue, double.parse(bean.cosusdt.price)),
              double.parse(bean.usdcny.price));
        } else {
          return 0;
        }
      } else if (lan == 'tw') {
        if (bean.usdtwd != null && !TextUtil.isEmpty(bean.usdtwd.price)) {
          return NumUtil.multiply(
              NumUtil.multiply(revenue, double.parse(bean.cosusdt.price)),
              double.parse(bean.usdtwd.price));
        } else {
          return 0;
        }
      } else if (lan == 'jp') {
        if (bean.usdjpy != null && !TextUtil.isEmpty(bean.usdjpy.price)) {
          return NumUtil.multiply(
              NumUtil.multiply(revenue, double.parse(bean.cosusdt.price)),
              double.parse(bean.usdjpy.price));
        } else {
          return 0;
        }
      } else if (lan == 'ko') {
        if (bean.usdkrw != null && !TextUtil.isEmpty(bean.usdkrw.price)) {
          return NumUtil.multiply(
              NumUtil.multiply(revenue, double.parse(bean.cosusdt.price)),
              double.parse(bean.usdkrw.price));
        } else {
          return 0;
        }
      } else if (lan == 'vi') {
        if (bean.usdvnd != null && !TextUtil.isEmpty(bean.usdvnd.price)) {
          return NumUtil.multiply(
              NumUtil.multiply(revenue, double.parse(bean.cosusdt.price)),
              double.parse(bean.usdvnd.price));
        } else {
          return 0;
        }
      } else if (lan == 'pt-br') {
        if (bean.usdbrl != null && !TextUtil.isEmpty(bean.usdbrl.price)) {
          return NumUtil.multiply(
              NumUtil.multiply(revenue, double.parse(bean.cosusdt.price)),
              double.parse(bean.usdbrl.price));
        } else {
          return 0;
        }
      } else if (lan == 'ru') {
        if (bean.usdrub != null && !TextUtil.isEmpty(bean.usdrub.price)) {
          return NumUtil.multiply(
              NumUtil.multiply(revenue, double.parse(bean.cosusdt.price)),
              double.parse(bean.usdrub.price));
        } else {
          return 0;
        }
      } else if (lan == 'tr') {
        if (bean.usdtry != null && !TextUtil.isEmpty(bean.usdtry.price)) {
          return NumUtil.multiply(
              NumUtil.multiply(revenue, double.parse(bean.cosusdt.price)),
              double.parse(bean.usdtry.price));
        } else {
          return 0;
        }
      } else {
  ...
  static double getStatusFinishTotalRevenueVest(String vest, String vestGift) {
    double giftVest = NumUtil.getDoubleByValueStr(vestGift ?? "0");
    double videoVest = NumUtil.getDoubleByValueStr(vest ?? "0");
    double originTotal = NumUtil.add(giftVest, videoVest);
    double totalVest = NumUtil.divide(
        originTotal, RevenueCalculationUtil.cosUnit);
    return totalVest;
  }
  ...
  static double calCurrentEnergy(String votePowerStr, String vestStr) {
    ...
    double result = NumUtil.multiply(votePower, vest);
    return result;
  }
  ...
  static double calLowestEnergyConsume(String vestStr) {
    ...
    double vest = NumUtil.getDoubleByValueStr(vestStr);
    double result = NumUtil.multiply(
        vest, RevenueCalculationUtil.lowestEnergyRatio);
    return result;
  }
  ...
  static double vestToEnergy(String vestStr) {
    ...
    double vest = NumUtil.getDoubleByValueStr(vestStr);
    double result = NumUtil.multiply(vest, RevenueCalculationUtil.energyUnit);
    return result;
  }

  static int calResumeLowestEnergySeconds(double currentEnergy,
      double maxEnergy) {
    if (maxEnergy <= 0) {
      return 0;
    }

    double lowestEnergy = NumUtil.multiply(
        maxEnergy,
        NumUtil.divide(
            RevenueCalculationUtil.lowestEnergyRatio,
            RevenueCalculationUtil.energyUnit));
    ...
    int second = NumUtil.multiply(NumUtil.divide(diff, maxEnergy), 24 * 3600)
        .ceil();
    return second;
  }

  static int calResumeLowestEnergyMinutes(double currentEnergy,
      double maxEnergy) {
    int seconds = RevenueCalculationUtil.calResumeLowestEnergySeconds(
